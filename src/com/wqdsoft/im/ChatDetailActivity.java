...
import com.wqdsoft.im.Entity.ChatDetailEntity;
...
import com.wqdsoft.im.adapter.ChatPersonAdapter;
...
import com.wqdsoft.im.global.SystemContactGlobal;
	...
	private void kickPerson(final int pos){
		new Thread(){
			...
			public void run(){
				if(IMCommon.verifyNetwork(mContext)){
					try {
						IMJiaState state = IMCommon.getIMInfo().kickParticipant(mGroupID, mList.get(pos).mLogin.uid);
			...
			public void run() {
				try {
					...
					if(type == 1){
						...
						if(mIsSignChat == 1){
							//setMsg
							stauts = IMCommon.getIMInfo().setMsg(mGroupID);
						}else{
