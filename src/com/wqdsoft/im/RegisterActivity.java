		...
		new Thread(){
			public void run() {
				try {
					...
					//++检测验证码++//
					IMJiaState validState = IMCommon.getIMInfo().checkVerCode(mInputNumber, mInputValidCode);
					//-检测验证码-//
					
					if(validState !=null && validState.code == 0){//注册
						LoginResult status = IMCommon.getIMInfo().register(mInputNumber, 
		...
		new Thread() {
			public void run() {
				try {
					...
					IMJiaState status = IMCommon.getIMInfo().getVerCode(mInputNumber,type);
