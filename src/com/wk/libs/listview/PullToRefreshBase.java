...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private void init(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.WKPullToRefresh);

		if (a.hasValue(R.styleable.WKPullToRefresh_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(
					R.styleable.WKPullToRefresh_ptrMode, 0));
		}

		if (a.hasValue(R.styleable.WKPullToRefresh_ptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.WKPullToRefresh_ptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.WKPullToRefresh_ptrRefreshableViewBackground)) {
			Drawable background = a
					.getDrawable(R.styleable.WKPullToRefresh_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a
				.hasValue(R.styleable.WKPullToRefresh_ptrAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground",
					"ptrRefreshableViewBackground");
			Drawable background = a
					.getDrawable(R.styleable.WKPullToRefresh_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.WKPullToRefresh_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(
					R.styleable.WKPullToRefresh_ptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.WKPullToRefresh_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a
					.getBoolean(
							R.styleable.WKPullToRefresh_ptrScrollingWhileRefreshingEnabled,
							false);
		}
